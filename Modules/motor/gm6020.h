...
typedef struct
{
    ...
    float angle_single_round_deg;  // 单圈角度,单位 deg
    float total_angle_deg;         // 累计总角度,调试器里常看的角度反馈量
    int32_t total_round;           // 已跨过的整圈数
    float speed_deg_s;             // 当前转速,单位 deg/s,调试器里常看的速度反馈量
...
typedef struct
{
    ...
    float angle_ref_deg; // 当前目标角度,调试器里常看的设定值
